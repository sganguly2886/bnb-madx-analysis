...
# Gather files
twiss_files = sorted(glob.glob("twiss_IQ874_*.out"))
groups = {
    "default": [],
    "10percentup": [],
    "10percentdown": [],
...
for file in twiss_files:
    base = os.path.basename(file).replace("twiss_IQ874_", "").replace(".out", "")
...
styles = {
    ...
    "10percentdown": ":",
    "20percentup": "-.",
    "20percentdown": (0, (3, 5, 1, 5))  # custom dashed-dot pattern
}
colormaps = {
...
for group, files in groups.items():
    ...
    for i, (file, label) in enumerate(sorted(files)):
        ...
        color = to_hex(cmap(i / max(n - 1, 1)))  # Normalize color scale
        plt.plot(df['S'], df['BETX'], label=f"{group} – {betx_label}", linestyle=linestyle, color=color, linewidth=1.5)

plt.title("BETX vs S for Varying HQ874 Currents and BETX")
plt.xlabel("S (m)")
plt.ylabel("BETX (m)")
plt.grid(True)
...
plt.savefig("betx_HQ874_readable.png", dpi=300)
plt.show()
